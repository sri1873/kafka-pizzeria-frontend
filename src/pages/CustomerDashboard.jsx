...
export default function CustomerDashboard() {
    ...
    const notification = useSSE(userId, "http://localhost:8080/customer/events");
                        ...
                        <div key={msg.orderId}
                            ...
                            <div className="flex items-center justify-between">
                                <p className="text-sm text-gray-500">
                                    {msg.orderDetails?.status === 'Delivered' && 'Your order has been delivered. Enjoy!'}
                                    {msg.orderDetails?.status === 'Preparing' && 'Your order is being prepared.'}
                                    {msg.orderDetails?.status === 'Cancelled' && 'This order was cancelled.'}
                                    {msg.orderDetails?.status === 'Confirmed' && 'Your order has been confirmed.'}
                                    {msg.orderDetails?.status === 'Ready' && 'Your order is ready for pickup.'}
                                </p>
                                {msg.lastUpdated && (
